...
namespace Service.Implementations
{
    public class SellerProfileService : ISellerProfileService
    {
        ...
        public async Task<SellerProfileDto?> GetByUserIdAsync(int userId)
        {
            var entity = await _sellerRepo.FirstOrDefaultAsync(s => s.UserId == userId);
            if (entity == null) return null;

            var dto = _mapper.Map<SellerProfile, SellerProfileDto>(entity);
            var user = await _userRepo.GetByIdAsync(userId);
            if (user != null)
            {
                dto.UserName = user.FullName;
            ...
            dto.StatusName = ((SellerStatus)entity.Status).ToString();
            return dto;
        }
